...
interface PublicTripStatsProps {
  trip: Trip;
  cloudLinkCount: number;
  className?: string;
}

export async function PublicTripStats({
  trip,
  cloudLinkCount,
  className,
}: PublicTripStatsProps) {
  const t = await getTranslations("publicTrip.stats");

  const stats = [
    { icon: CalendarDays, value: trip.stats.dayCount, label: t("days") },
    { icon: List, value: trip.stats.eventCount, label: t("moments") },
    { icon: Camera, value: trip.stats.photoCount, label: t("photos") },
    ...(cloudLinkCount > 0
      ? [{ icon: Cloud, value: cloudLinkCount, label: t("cloudLinks") }]
      : []),
    ...(trip.stats.placeCount > 0
      ? [{ icon: MapPinned, value: trip.stats.placeCount, label: t("places") }]
      : []),
